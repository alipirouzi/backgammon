...
host's forced-command SSH pipeline) and the **engine** exists: `bg-core`
...
analysis output; `bg-wasm` (WebAssembly) and `bg-node` (native Node.js addon)
expose one JSON API over them, each held to the shared test vectors by a
parity test, and the Docker image builds `bg-wasm` in a Rust stage (see
[Engine](#engine) and [Bindings](#bindings-bg-wasm-bg-node)). Nothing plays
backgammon in the browser yet: the web app depends on `bg-wasm` but no page
uses it until the Play piece `(planned)`.
...
│   ├── tests/                       Vitest unit tests, incl. engine-parity.test.ts (bg-wasm against engine/vectors)
...
Full local gate (CI additionally runs `pnpm test -- --coverage` and Playwright
e2e). Order matters twice: `wasm-pack build` before `pnpm install`, and
`pnpm --filter bg-node build` before the root `pnpm test`:
...
- Image smoke test: `docker run -d --rm --name bg-smoke -p 3999:3000 backgammon:local`, then `curl -fsS http://127.0.0.1:3999/health` (expect `{"status":"ok"}`) and `curl -fsS http://127.0.0.1:3999/ | grep -c 'id="board-mount"'`; `docker rm -f bg-smoke` afterwards.
...
(Turbopack) loads natively; no page imports it yet — the board, worker and
analysis UI are the Play piece `(planned)`. Today the only consumer is the
parity test.
...
- `ci.yml` runs on every push and PR: job `engine` (`cargo fmt --check`,
  ...
  `bg-wasm/tests/vectors.rs`). The screenshot matrix from the spec is
  `(planned)`.
